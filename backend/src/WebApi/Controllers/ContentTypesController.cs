...
public class ContentTypesController(IMediator mediator) : ControllerBase
{
	...
	public async Task<OutcomeResult<ContentTypeDto>> Create(
		CreateContentTypeDto body,
		CancellationToken cancellationToken) =>
		await mediator.Send(new CreateContentTypeCommand(body.Name, body.Kind, body.Fields), cancellationToken);

	[HttpPatch("{id:guid}")]
	public async Task<OutcomeResult<ContentTypeDto>> Update(
		Guid id,
		UpdateContentTypeDto body,
		CancellationToken cancellationToken) =>
	...
	public async Task<OutcomeResult<ContentFieldDto>> AddField(
		Guid id,
		CreateContentFieldDto field,
		CancellationToken cancellationToken) =>
		await mediator.Send(new AddFieldToContentTypeCommand(id, field), cancellationToken);

	[HttpPatch("{id:guid}/fields/{fieldId:guid}")]
	public async Task<OutcomeResult<ContentFieldDto>> UpdateField(
		Guid id,
		Guid fieldId,
	...
	public async Task<OutcomeResult<ContentFieldDto>> RemoveField(
		Guid id,
		Guid fieldId,
		CancellationToken cancellationToken) =>
